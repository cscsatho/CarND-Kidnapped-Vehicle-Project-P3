...
int main()
{
  ...
  ParticleFilter pf(/* num_particles */ 25, sigma_pos, sigma_landmark);

  h.onMessage([&pf,&map,&delta_t,&sensor_range](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  pf.updateWeights(sensor_range, noisy_observations, map);
	  pf.resample();
